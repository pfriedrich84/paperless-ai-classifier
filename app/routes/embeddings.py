"""Embeddings dashboard — vector DB inspection, similarity search, and webhook."""
...
from app.routes.webhook import _extract_document_id, _verify_webhook_secret
...
# ---------------------------------------------------------------------------
# Webhook: re-embed a document (no classification)
# ---------------------------------------------------------------------------
@router.post("")
async def embeddings_webhook(
    request: Request,
    x_webhook_secret: str | None = Header(default=None),
):
    """Re-embed a single document triggered by a Paperless webhook.

    Unlike the full ``/webhook/paperless`` endpoint this only recomputes the
    document's embedding (with optional OCR correction) — no classification,
    no suggestion, no Telegram notification.

    Accepts Paperless workflow webhook payloads
    (``{"event": "...", "object": {"id": ...}}``) and legacy
    payloads (``{"document_id": ...}``).
    """
    auth_error = _verify_webhook_secret(x_webhook_secret)
    if auth_error:
        log.warning("embeddings webhook auth failed")
        return auth_error

    body = await request.json()
    doc_id = _extract_document_id(body)
    if doc_id is None:
        log.warning("embeddings webhook payload missing document id", payload=body)
        return JSONResponse(
            status_code=422,
    ...
    if is_reindexing():
        log.info("reindex in progress — rejecting embeddings webhook", document_id=doc_id)
        return JSONResponse(
            status_code=503,
    ...
    log.info("embeddings webhook triggered", document_id=doc_id, webhook_event=body.get("event"))
    ...
    try:
        ...
        log.error("embeddings webhook failed", document_id=doc_id, error=str(exc))
        return JSONResponse(
            status_code=500,
